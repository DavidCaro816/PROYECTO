...
class database
{
    ...
    public function recent_data() :array{
        $recent_data = $this->connection->query("SELECT cotizacion.idCotizacion,CONCAT(primerNombre,' ',segundoNombre,' ',primerApellido,' ',segundoApellido) AS nombre,seguro.seguro,GROUP_CONCAT(aseguradora.logo) AS logo FROM cotizacion,seguro,cliente,aseguradoracotizante,aseguradora WHERE cotizacion.idSeguro = seguro.idSeguro AND cotizacion.idCotizacion = aseguradoracotizante.idCotizacion AND cliente.documento = cotizacion.documento AND aseguradoracotizante.idAseguradora = aseguradora.idAseguradora GROUP BY cotizacion.idCotizacion");
        $recent_data->execute();
        return $recent_data->fetchAll(PDO::FETCH_ASSOC);
    }

}
(new database())->recent_data();